...
## 基于网格几何的浮力 / 水动力学脚本
## ============================================================
## 算法原理（参考 q-qo-o/isaac-sim-hydrodynamics）：
##
...
##
## 为什么这比体素采样更好：
##   水线交点是解析求解的，所以浸没体积和浮心位置关于物体
##   姿态是 C⁰ 连续的。倾斜物体从水中弹起时，浮心平滑地
##   滑回几何中心，不会出现"最后几个体素跳变"导致的剧烈力矩。
##
## 使用要求：
##   - 节点本身必须是 RigidBody3D。
...
func _integrate_forces(state: PhysicsDirectBodyState3D) -> void:
	...
	if absf(total_submerged_volume) > 1e-9:
		...
		var buoyancy_force := Vector3.UP * submerged_volume * fluid_density * _gravity
